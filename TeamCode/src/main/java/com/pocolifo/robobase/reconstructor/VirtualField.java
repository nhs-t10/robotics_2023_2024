...
import com.pocolifo.robobase.utils.UnitUtils;
...
public class VirtualField {
        ...
        try (InputStream resourceAsStream = this.getClass().getResourceAsStream(virtualFieldResourcePath)) {
            ...
            for (int x = 0; width > x; x++) {
                for (int y = 0; height > y; y++) {
                    ...
                    double pointX = ByteBuffer.wrap(content, i+1, 8).order(ByteOrder.LITTLE_ENDIAN).getDouble();
                    double pointY = ByteBuffer.wrap(content, i+1+8, 8).order(ByteOrder.LITTLE_ENDIAN    ).getDouble();
    ...
    public static TrajectorySequence trajectify(List<Point> points, TrajectorySequenceBuilder builder) {
    ...
    public Point getNearestPoint(double pX, double pY) {
        Point closest = null;
    ...
    public Point getNearestPoint(Pose2d pose2d) {
        return getNearestPoint(UnitUtils.inchesToMeters(pose2d.getX()), UnitUtils.inchesToMeters(pose2d.getY()));
    }
